...
def test_browseros_block_gets_soft_block_json_with_alternatives():
    ...
    assert flow.response.headers["Content-Type"].startswith("text/html")
    assert flow.response.headers["X-AgentGuard-Decision"] == "block"
    assert flow.response.headers["X-AgentGuard-Continuation"] == "available"
    assert flow.response.headers["X-AgentGuard-Recovery-Mode"] == "guided"
    text = flow.response.content.decode("utf-8")
    assert "Continue the same task using a safer alternative below." in text


def test_browseros_block_non_navigation_gets_json_soft_block():
    flow = _make_flow()
